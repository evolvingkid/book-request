...
if (isset($_POST['submit'])) {
  if ($_POST['cash_type'] == 1) {
    ...
    VALUES ('$_GET[id]', 'cash on front', '$_POST[category_name]', '$_SESSION[dept_id]')";
    ...
    //   echo "Error: " . $sql . "<br>" . $conn->error;
     ...
     header('location:bank.php?id='.$_GET['id'].'&qty='.$_POST['category_name']);
  }
}
 ?>
...
<meta name="viewport" content="width=device-width, initial-scale=1">
  ...
  <div class="left input_txt">Qty:</div>
  <input type="number" name="category_name" value="" class="input_Style_1" placeholder="Quantity" required>
            </div>
            <div class="form_css left">
  <div class="left input_txt">Payment Method:</div>
  <select class="" name="cash_type">
    <option value="1">Cash on Front</option>
    <option value="2">net backing</option>
  </select>
            </div>
              </div>
                      <div class="float_covering">
